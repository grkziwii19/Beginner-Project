...
import {
  type Student, type ClassItem, type Gender, type AttendanceStatus, type GradeType,
  getStatusColor, getStatusLabel, getPredicate, getPredicateColor
} from '@/types'
import {
  ArrowLeft, Plus, Pencil, Trash2, X, Users, ChevronLeft, ChevronRight,
...
export default function ClassDetailPage() {
  ...
  const handleSaveStudent = async () => {
    if (!studentForm.name || !studentForm.nis || !classItem) return
                      ...
                      <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="table-cell">
                          <div className="flex items-center gap-1">
                            <button onClick={() => openEditStudent(s)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <h2 className="font-semibold text-slate-900">{editStudent ? 'Edit Siswa' : 'Tambah Siswa'} — {classItem.name}</h2>
              <button onClick={() => setShowStudentModal(false)} className="p-1 text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <select className="input" value={studentForm.gender}
                  onChange={e => setStudentForm({ ...studentForm, gender: e.target.value as Gender })}>
            ...
            <div className="flex gap-3 p-5 border-t border-slate-100">
              <button onClick={() => setShowStudentModal(false)} className="btn-secondary flex-1 justify-center">Batal</button>
              <button onClick={handleSaveStudent} disabled={savingStudent} className="btn-primary flex-1 justify-center">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDeleteStudent(deleteConfirm)} className="btn-danger flex-1 justify-center">Hapus</button>
            </div>
          </div>
        </div>
      )}
