  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (validRows.length === 0) {
        setImportMsg({ type: 'error', text: 'Tidak ada data valid ditemukan. Pastikan format: Nama, NIS, Kelas, Jenis Kelamin.' })
        return
      }
      ...
      setImportMsg({ type: 'success', text: `${validRows.length} siswa berhasil diimport!` })
    } catch (err: any) {
    ...
    <div className="space-y-6">
        ...
        <div className="max-w-xl space-y-4">
          <div className="card p-6">
            ...
            <p className="text-sm text-slate-500 mb-4">
              Upload file Excel (.xlsx) berisi data siswa. Pastikan kolom sesuai urutan: Nama, NIS, Kelas, Jenis Kelamin.
            </p>
            ...
            <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-slate-200 rounded-xl p-8 cursor-pointer hover:border-emerald-300 hover:bg-emerald-50/30 transition-colors">
              <Upload className="w-8 h-8 text-slate-300" />
              <span className="text-sm font-medium text-slate-600">
                {importing ? 'Mengimport...' : 'Klik untuk pilih file Excel'}
              </span>
      ...
      {activeTab === 'export' && (
        <div className="max-w-xl space-y-3">
          {[
            { type: 'students' as const, label: 'Data Siswa', desc: 'Export semua data siswa terdaftar', icon: Users, color: 'bg-blue-50 text-blue-600' },
            ...
            <div key={item.type} className="card p-4 flex items-center gap-4">
              ...
              >
                ...
                {exporting === item.type ? 'Mengexport...' : 'Export'}
              </button>
            </div>
          ))}
        </div>
